...
Route::group([
    ...
    // Log Out
    Route::get('/account/log-out', [App\Http\Controllers\Account\SettingsController::class, 'log_out'])->name('account-log-out');

    // --------------------- Users -------------------
    Route::get('/user/{id}', [App\Http\Controllers\UsersController::class, 'index'])->name('users');

    // --------------------- Filter ---------------------
    Route::any('/filter/{category_id?}/{min_price?}/{max_price?}/{location?}/{post_type?}/{auth_type?}/{search_value?}/{request_type?}', [App\Http\Controllers\ItemController::class, 'filter'])->name('filter');
